...
def patch_toggle(text: str) -> str:
    ...
    new = """            // AYU_UNLIMITED_PINS_v2: runtime switch. OFF restores Telegram's\n            // normal client-side limits; ON keeps overflow pins local to this device.\n            let limitCount: Int\n            if AyuUnlimitedPins.isEnabled {\n                limitCount = Int.max\n            } else if case .root = groupId {\n                limitCount = Int(userLimitsConfiguration.maxPinnedChatCount)\n            } else {\n                limitCount = Int(userLimitsConfiguration.maxArchivedPinnedChatCount)\n            }\n"""
    return replace_once(text, old, new, "toggle-runtime-limit")
...
def main() -> None:
    ...
    print("[ayu-unlimited-pins] Runtime toggle is available in AyuGram -> Chats.")
